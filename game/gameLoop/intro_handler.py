# game/gameLoop/intro_handler.py
import pygame
from ..ui import draw_intro_screen
...
class IntroHandler:
    def __init__(self, screen, audio):
        ...
        self.intro_duration = 35000
        self.fade_in_duration = 5000

    async def run(self):
        intro_start_time = pygame.time.get_ticks()
        self.audio.play_bgm("audio/TBC/000.ogg")
        ...
        while True:
            ...
            if elapsed >= self.fade_in_duration:
                progress = min(1.0, (elapsed - self.fade_in_duration) / (self.intro_duration - self.fade_in_duration))
                y_offset = self.screen.get_height() * (1 - progress * progress)
            skip_rect = draw_intro_screen(self.screen, self.font, y_offset, alpha)
            pygame.display.flip()
            ...
            if elapsed >= self.intro_duration + DELAY_TIME:
                return "level_selection"
